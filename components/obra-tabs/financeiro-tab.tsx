...
interface LancRow {
  ...
  fornecedor_nome: string; centro_custo_nome: string
}
...
export function FinanceiroTab({ obraId }: FinanceiroTabProps) {
  ...
  const load = useCallback(async () => {
    ...
    const { data: lancs } = await supabase.from("lancamento").select("*").eq("id_obra", obraId).order("data_competencia", { ascending: false })
    const list = (lancs ?? []) as (LancRow & { id_fornecedor: number | null; id_centro_custo: number })[]
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold">Lançamentos da obra</h3>
        <Button size="sm" onClick={() => setFormOpen(true)}><Plus className="mr-2 h-4 w-4" /> Novo lançamento</Button>
      </div>
      ...
      <DataTable columns={columns} data={data} searchKey="historico" searchPlaceholder="Buscar por histórico..." isLoading={isLoading} emptyMessage="Nenhum lançamento." />
