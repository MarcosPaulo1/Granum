...
export default function FornecedorPerfilPage() {
  ...
  async function load() {
    ...
    const { data: lancs } = await supabase
      .from("lancamento")
      .select("id_lancamento, data_competencia, historico, valor")
      .eq("id_fornecedor", id)
      .eq("tipo", "realizado")
      .order("data_competencia", { ascending: false })

    const items = (lancs ?? []) as LancamentoResumido[]
            ...
            {lancamentos.map((l) => (
              <div key={l.id_lancamento} className="flex items-center justify-between border rounded-md p-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{formatDate(l.data_competencia)}</p>
                </div>
